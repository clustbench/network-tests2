		...
		for(proc2=0;proc2<comm_size; proc2++)
		{
			...
			for(i=0;i<num_repeats;i++)
			{
				...
				if(mode_array[0]==MODE_GOAL_MESSAGES)
				{
					...
					if(proc2!=0)
					{
						MPI_Send( td.send_data[remote_proc], mes_length, MPI_BYTE, remote_proc, 0, MPI_COMM_WORLD);
					}
					else
					{
						time_beg=px_my_cpu_time();
							MPI_Recv( td.recv_data[remote_proc], mes_length, MPI_BYTE, remote_proc, 0, MPI_COMM_WORLD, &status);

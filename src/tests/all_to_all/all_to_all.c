...
int all_to_all(clustbench_time_result_t *times,  clustbench_time_t *real_times, int mes_length,int num_repeats, void *additional)
{
    ...
    for(i=0; i<num_repeats; i++)
    {
        ...
        // 1: Как сюда встроить алгоритм?
        // 2: почему после цикла с MPI_Waitany нет синхронизации, здесь же нет гарантии одновременности выхода из цикла...
    }
...
int all_to_all_mash(clustbench_time_result_t **times, clustbench_time_t **real_times,
                                int num_repeats,
                                int beg_length,
                                int step_length,
                                int end_length,

...
int create_netcdf_header4d_eq_class
(
	const struct network_test_parameters_struct *test_parameters,
	int *file_id,
	int *data_id,
...
)
{
	...
	if(nc_create(filename,NC_NOCLOBBER|NC_SHARE|NC_64BIT_OFFSET,&netcdf_file_id)!=NC_NOERR)
	{
		return CREATE_FILE_ERROR;
	}

	if(nc_def_dim(netcdf_file_id,"send",test_parameters->num_procs,&x_dim_id)!=NC_NOERR)
	{
		return NETCDF_ERROR;
	}

	if(nc_def_dim(netcdf_file_id,"recv",test_parameters->num_procs,&y_dim_id)!=NC_NOERR)
	{
		return NETCDF_ERROR;
	}

	if(nc_def_dim(netcdf_file_id,"repeats",test_parameters->num_repeats,&z_dim_id)!=NC_NOERR)
	{
		return NETCDF_ERROR;
	}

	if(nc_def_dim(netcdf_file_id,"mes_length",NC_UNLIMITED,&num_matrices_dim_id)!=NC_NOERR)
	{
		return NETCDF_ERROR;
	}
